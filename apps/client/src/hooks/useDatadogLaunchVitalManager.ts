...
export const useDatadogLaunchVitalManager = (
    ...
    const stopVital = useCallback(
        (context: VitalStopContext): void => {
            ...
            if (isValidVitalRef(vitalRef)) {
                ...
                if (context.status === "success") {
                    ...
                    logger.warn("Datadog vital stopped with error", {
                        error: context.error,
                    })
                }
            } else {
    ...
    const stopVitalOnSuccess = useCallback((): void => {
        stopVital({ status: "success" })
    }, [stopVital])

    const stopVitalOnError = useCallback(
        (error: Error): void => {
    ...
    const stopVitalOnUnmount = useCallback((): void => {
        if (vitalRef && !vitalStoppedRef.current) {
            stopVital({
                status: "error",
    ...
    return {
        ...
        stopVitalOnError,
        stopVitalOnUnmount,
        get isVitalStopped(): boolean {
            return vitalStoppedRef.current
        },
    }
}
